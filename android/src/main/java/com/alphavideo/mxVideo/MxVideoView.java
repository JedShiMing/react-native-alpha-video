...
public class MxVideoView extends GLTextureView {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    mWidth = widthSize;
    mHeight = heightSize;
    //    Log.d("AlphaVideo", "onMeasure mWidth " + mWidth + "  mHeight " + mHeight + " videoAspectRatio " + videoAspectRatio);
    ...
    if (mScaleType == CENTER_INSIDE) {
      ...
      if (mVideoWidth > 0 && mVideoHeight > 0) {
        ...
        // 这个时候以高为基准计算实际的宽，然后再做平移
        float scale = (float) mVideoHeight / mVideoWidth;
        int rstWidth = (int) (mHeight / scale);
        //        super.onMeasure(width, mHeight);
        setMeasuredDimension(rstWidth, mHeight);
        int deslocationX = (rstWidth - mWidth) / 2;
        this.animate().translationX(-deslocationX);
        return;
      }
      if (currentAspectRatio > videoAspectRatio) {
        heightSize = (int) (widthSize / videoAspectRatio);
  ...
  private void onDataSourceSet(MediaMetadataRetriever retriever) {
    ...
    //    Log.d("AlphaVideo", "onDataSourceSet w " + videoWidth + "  h " + videoHeight);
